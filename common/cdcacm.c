...
led_control(usbd_device *usbd_dev, struct usb_setup_data *req, uint8_t **buf,
...
static void cdcacm_data_rx_cb(usbd_device *usbd_dev, uint8_t ep) {
  ...
  int len = usbd_ep_read_packet(usbd_dev, 0x01, sdram_ptr, 64);
...
static void cdcacm_set_config(usbd_device *usbd_dev, uint16_t wValue) {
  ...
  usbd_register_control_callback(
      usbd_dev, USB_REQ_TYPE_VENDOR | USB_REQ_TYPE_INTERFACE,
      USB_REQ_TYPE_TYPE | USB_REQ_TYPE_RECIPIENT, led_control);
}
...
usbd_device *create_usb_handle(void) {
  ...
  usbd_device *usb_dev =
      usbd_init(&otghs_usb_driver, &dev, &config, usb_strings, 3,
