...
void lcd_draw_pixel_1d(int index, uint16_t color) {
  if (index >= LCD_WIDTH * LCD_HEIGHT) {
    // printf("Draw out of range [%d, %d]\n", x, y);
    while (1)
      ;
  }

  *(cur_frame + index) = color;
}
...
static const uint8_t cmd_args[] = {
    ...
    0x01,
    0x00,
    0x00, /* modified to remove RGB mode */
    0x01,
    0x0F,
    0x29,
    0x24,
...
void lcd_init(void) {
  ...
  spi_init_master(
      LCD_SPI, SPI_CR1_BAUDRATE_FPCLK_DIV_4, SPI_CR1_CPOL_CLK_TO_0_WHEN_IDLE,
